...
/*
		if( true || isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false) ){
			echo '<div style="clear:both;padding-top:1em;"></div>';
			echo '<h4>It Looks Like You\'re Still Using Internet Explorer?</h4>';
			echo '<p>';
			echo 'Web developers everywhere recommend using a browser other than Internet Explorer for security and compatabilty reasons.';
			echo '</p>';
			echo '<p>Try one of the following successful browsers</p>';
			echo '<ul>';
			echo '<li><a href="http://www.firefox.com">Firefox</a></li>';
			echo '<li><a href="http://www.google.com/chrome">Google Chrome</a></li>';
			echo '<li><a href="http://www.opera.com">Opera</a></li>';
			echo '</ul>';
...
*/
	...
	function RunAdminScript(){
		...
		switch($this->requested){
			case 'Admin_Browser':
				require($rootDir.'/include/admin/admin_browser.php');
				new admin_browser();
			return;
			
			case 'Admin_Password':
				require($rootDir.'/include/admin/admin_password.php');
				new admin_password();
			return;
			
			case 'Admin_About':
				require($rootDir.'/include/admin/admin_about.php');
				new admin_about();
			return;
		}
	...
	function SendPassword(){
		...
		$users[$username]['newpass'] = sha1(trim($newpass));
...
*/
		...
		echo '<div style="display:none;" class="req_script">';
			...
			echo '<tr><td>';
				echo '</td><td>';
					echo '<input type="submit" name="aa" value="'.$langmessage['login'].'" />';
				echo '</td></tr>';
			echo '</table>';
		echo '</div>';
