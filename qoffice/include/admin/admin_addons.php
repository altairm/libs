...
class admin_addons extends admin_addon_install{
	...
	function RatingFunctions($cmd){
		...
		//User can rate addons they have installed and addons they have recently uninstalled
		if( isset($rate_info['addonDir']) ){
			$this->ShowAddon($rate_info['addonDir']);
		}else{
			$this->Select();
		}
	...
	*/
	function History(){
		global $langmessage;
		echo '<h2>'.$langmessage['addon_history'].'</h2>';
		$this->ShowHistory();
	}
	
	function ShowHistory($rows = 30 ){
		global $langmessage;
		...
		$array = array_reverse($this->addonHistory,true);
		$i = 0;
		foreach($array as $data){
			echo '<tr>';
			echo '<td>';
			echo date('M j, Y \a\t G:i',$data['time']);
			echo '</td>';
			echo '<td>';
			echo $data['name'];
			echo '</td>';
			...
			if( isset($data['id']) ){
				$can_rate = $this->CanRate($data['id'],$rate_info);
				if( $can_rate === true ){
					echo common::Link('Admin_Addons',$langmessage['rate'],'cmd=rate&arg='.$rate_info['id']);
				}else{
					echo $can_rate;
				}
	...
	*/
	...
	function Uninstall(){
		...
		if( !isset($config['addons'][$addon]) ){
			...
			$mess .= ' <input type="submit" name="aaa" value="'.$langmessage['continue'].'" /> ';
	...
	function ShowAddon($addon=false){
		global $config,$langmessage,$page,$dataDir;
		...
		echo '<h2>'.$langmessage['add-ons'].': '.$config['addons'][$addon]['name'].' ('.$addon.')</h2>';

		
		//show Special Links
			$sublinks = admin_tools::GetAddonSubLinks_New('special',$addon);
			if( !empty($sublinks) ){
				echo '<h4>Special Links</h4>';
				echo '<ul>';
				echo $sublinks;
				echo '</ul>';
			}
		...
		//show Admin Links
			//$links = admin_tools::GetAddonComponents($config['admin_links'],$addon);
		...
		if( $this->CanRate($addon,$rate_info) ){
	...
	function Select(){
		...
		if( admin_tools::CanBrowseAddons() ){
			echo '<p>';
			echo common::Link('Admin_Addons','Browse Additional Addons','cmd=browse');
			echo '</p>';
		}
		
		
/*
		if( count($this->addonHistory) > 0 ){
			echo '<h3>'.$langmessage['addon_history'].'</h3>';
			$this->ShowHistory(3);

			echo common::Link('Admin_Addons',$langmessage['view_entire_history'],'cmd=history');
		}
*/
	...
	function ShowInstalled(&$available){
		...
		foreach($show as $folder => $info){
			...
			if( $this->CanRate($folder,$rate_info) === true ){
				echo common::Link('Admin_Addons',$langmessage['rate'],'cmd=rate&arg='.$rate_info['id']);
			}else{
			...
			//upgrade link
				...
				if( $developerInstall ){
					echo common::Link('Admin_Addons',$label,'cmd=step1&mode=dev&addon='.$folder);
				}else{
					echo common::Link('Admin_Addons',$label,'cmd=step1&addon='.$folder);
				}
