...
class thumbnail{
	...
	*/
	...
	/* static */
	function createSquare($source_path,$dest_path,$size=50,$file_type=false){
		$new_w = $new_h = $size;
		
		$src_img = thumbnail::getSrcImg($source_path,$file_type);
		if( !$src_img ){
			return false;
		}
...
*/
	...
	function getSrcImg($source_path,$type_file=false){
		...
		if( $type_file !== false ){
			$file_type = thumbnail::getType($type_file);
		}else{
			$file_type = thumbnail::getType($source_path);
		}
	...
	function createImg($src_img, $dest_path, $dst_x, $dst_y, $off_w, $off_h, $new_w, $new_h, $old_x, $old_y){
		...
		$file_type = thumbnail::getType($dest_path);
