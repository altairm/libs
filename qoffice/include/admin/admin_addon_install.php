...
class admin_addon_install extends admin_addons_tool{
	...
	function admin_addon_install($cmd){
		...
		$this->GetAddonData(); //addonHistory and addonReviews
...
*/
	...
	function AddToHandlers($gadgets){
		...
		if( !isset($config['theme_handlers']) || !is_array($config['theme_handlers']) || !is_array($gadgets) ){
			return;
		}
		
		foreach($config['theme_handlers'] as $theme => $containers){
			if( !is_array($containers) ){
				continue;
			}
			
			if( isset($containers['GetAllGadgets']) ){
				$container =& $config['theme_handlers'][$theme]['GetAllGadgets'];
	...
	//remove gadgets from config['theme_handlers']
	function RemoveFromHandlers($gadgets){
		...
		foreach($config['theme_handlers'] as $theme => $containers){
			...
			foreach($containers as $container => $handlers){
				...
				$config['theme_handlers'][$theme][$container] = $handlers;
			}
		}
	}
